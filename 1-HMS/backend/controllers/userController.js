...
// gateWay Initialize
const stripInstance = new strip(process.env.STRIPE_SECRET_KEY);
...
const registerUser = async (req, res) => {
  try {
    ...
    if (password.length < 8) {
      ...
      const salt = await bcrypt.henSalt(10); // the more no. round the more time it will take
...
// Api to get user appointments from frontend my-appointments pages
...
// Api to verify payment and update appointment data of razorpay
...
const paymentStripe = async (req, res) => {
  try {
    ...
    const currency = process.env.CURRENCY.toLocalLowerCase();
